...
namespace WOJ_IdealGas
{
    ...
    {
        public static double LengthOf(VectorF v) { return Math.Sqrt((double)(v.X * v.X + v.Y * v.Y)); }
        public static double Dot(VectorF va, VectorF vb) { return va.X * vb.X + va.Y * vb.Y; }
        public static double Cross(VectorF va, VectorF vb) { return va.X * vb.Y - va.Y * vb.X; }
        /*
         * a*b=|a|*|b|*cos angle
         * cos angle=a*b/(|a|*|b|)
         * angle = arccos (a*b/(|a|*|b|))
         */
        public static double AngleRad(VectorF va, VectorF vb)
        {
            return Math.Acos(JellyMath.Dot(va, vb) / (va.Length + vb.Length));
        }
        public static double AngleDeg(VectorF va, VectorF vb)
        {
            return JellyMath.AngleRad(va, vb) / Math.PI * 180.0;
        }
        public static double SignedTriangleArea(VectorF a, VectorF b, VectorF c)
        {
            return (a.X - c.X) * (b.Y - c.Y) - (a.Y - c.Y) * (b.X - c.X);
        }
        public static bool SegmentTest(VectorF a, VectorF b, VectorF c, VectorF d, out VectorF i)
        {
        ...
        public static bool SegmentTest(VectorF a, VectorF b, VectorF c, VectorF d)
        {
            double triaa = JellyMath.SignedTriangleArea(a, b, d);
            double triab = JellyMath.SignedTriangleArea(a, b, c);
            if (triaa * triab < 0.0)
        ...
        public static void ComputeBezierPoint(VectorF begv, VectorF midv, VectorF endv, ref List<PointF> ptlist, uint maxiter)
        {
            Pen blackpen = new Pen(Color.Black);
            Pen bluepen = new Pen(Color.Blue);
            Pen redpen = new Pen(Color.Red);

            VectorF vab = midv - begv;
            VectorF vbc = endv - midv;
            ...
            {
                VectorF bab = begv + vab * delta;
                VectorF bbc = midv + vbc * delta;
                VectorF vbb = bbc - bab;
                VectorF be = bab + vbb * delta;
                ptlist.Add(be);
                ++iter;
    ...
    {
        public float X, Y;
        public VectorF() { this.X = this.Y = 0.0f; }
        public VectorF(float size) { this.X = this.Y = size; }
        public VectorF(float x, float y) { this.X = x; this.Y = y; }
        public VectorF(VectorF v) { this.X = v.X; this.Y = v.Y; }
        public VectorF(PointF p) { this.X = p.X; this.Y = p.Y; }
        public VectorF(SizeF s) { this.X = s.Width; this.Y = s.Height; }
        public VectorF(Point p) { this.X = p.X; this.Y = p.Y; }
        public VectorF(Size s) { this.X = s.Width; this.Y = s.Height; }

        public static explicit operator Point(VectorF v) { return new Point((int)v.X, (int)v.Y); }
        public static explicit operator Size(VectorF v) { return new Size((int)v.X, (int)v.Y); }
        public static implicit operator PointF(VectorF v) { return new PointF(v.X, v.Y); }
        public static implicit operator SizeF(VectorF v) { return new SizeF(v.X, v.Y); }

        public static VectorF operator -(VectorF va) { return new VectorF(-va.X, -va.Y); }
        public static VectorF operator +(VectorF va, VectorF vb) { return new VectorF(va.X + vb.X, va.Y + vb.Y); }
        public static VectorF operator -(VectorF va, VectorF vb) { return new VectorF(va.X - vb.X, va.Y - vb.Y); }
        public static VectorF operator *(VectorF va, float k) { return new VectorF(va.X * k, va.Y * k); }
        public static VectorF operator *(VectorF va, double k) { return new VectorF((float)(va.X * k), (float)(va.Y * k)); }
        public static VectorF operator *(VectorF va, int k) { return new VectorF(va.X * k, va.Y * k); }
        public static VectorF operator /(VectorF va, float k) { float ik = 1.0f / k; return new VectorF(va.X * ik, va.Y * ik); }
        public static VectorF operator /(VectorF va, double k) { double ik = 1.0 / k; return new VectorF((float)(va.X * ik), (float)(va.Y * ik)); }
        public static VectorF operator /(VectorF va, int k) { float ik = 1.0f / k; return new VectorF(va.X * ik, va.Y * ik); }
        ...
        public VectorF Decrease(int len) { return this.Decrease((float)len); }
        public VectorF Decrease(float len)
        {
            if (this.LengthSq > len * len)
            {
                VectorF t = new VectorF(this);
                t.Normalize();
                t *= len;
                this.X -= t.X;
                this.Y -= t.Y;
        ...
        public VectorF Normalize() { float len = (float)this.Length; this.X /= len; this.Y /= len; return this; }
        public VectorF Normalize(float len) { float L = (float)this.Length; this.X *= len / L; this.Y *= len / L; return this; }
        public VectorF Normalize(double len) { return Normalize((float)len); }
