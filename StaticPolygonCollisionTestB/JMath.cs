...
namespace StaticPolygonCollisionTestB
{
    public static class JMath
    {
        ...
        public static bool SegmentPolygonTest(Vector a, Vector b, Vertex[] poly, out int id, out Vector ins)
        {
            ...
            return false;
            */
            for (id = 0; id < poly.Length - 1; )
                if (SegmentTest(a, b, poly[id].Position, poly[++id].Position, out ins))
                    return true;
            if (SegmentTest(a, b, poly[id].Position, poly[0].Position, out ins))
                return true;
            id = int.MaxValue;
            return false;
        }
        ...
        public static Vector ClosestPointOnSegment(Vector segb, Vector sege, Vector pt)
        {
            Vector ab = sege - segb;
            double t = Dot(pt - segb, ab) / Dot(ab, ab);
            if (t < 0) return segb;
            if (t > 1) return sege;
            return segb + ab * t;
        }
