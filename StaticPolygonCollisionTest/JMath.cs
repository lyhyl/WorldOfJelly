...
namespace StaticPolygonCollisionTest
{
    public static class JMath
    {
        ...
        public enum InOutTestResult { In, Out }
        public static InOutTestResult PointInOutTest(Vertex[] vs, Vector pt)
        {
            //AABB aabb = new AABB(vs);
            //Vector low = new Vector(pt.X + 0.001f, -1000.0f);//float.MinValue
            Vector low = new Vector(-12345.0f, -54321.0f);
            uint count = 0;
            uint i;
            for (i = 0; i < vs.Length - 1; ++i)
            {
                if (JMath.SegmentTest(pt, low, vs[i].Position, vs[i + 1].Position))
                    ++count;
            }
            if (JMath.SegmentTest(pt, low, vs[vs.Length - 1].Position, vs[0].Position))
                ++count;
            return (count & 1) == 1 ? InOutTestResult.In : InOutTestResult.Out;
        }
