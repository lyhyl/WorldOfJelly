...
namespace JellyTetris.JellyWorld.Jelly
{
	...
	{
        public JellyLong(GraphicsDevice gd, float mass, float size, Color color, JellyVector2 pos)
            : base(gd, mass, size, color, 10)
		{
			//nodes
			_nodes = new JellyVertex[14];
			_nodes[0] = new JellyVertex(new JellyVertex(pos.X + -4 * size, pos.Y + 1 * size));
			_nodes[1] = new JellyVertex(new JellyVertex(pos.X + -2 * size, pos.Y + 1 * size));
			_nodes[2] = new JellyVertex(new JellyVertex(pos.X + 0 * size, pos.Y + 1 * size));
			_nodes[3] = new JellyVertex(new JellyVertex(pos.X + 2 * size, pos.Y + 1 * size));
			_nodes[4] = new JellyVertex(new JellyVertex(pos.X + 4 * size, pos.Y + 1 * size));
			_nodes[5] = new JellyVertex(new JellyVertex(pos.X + -3 * size, pos.Y + 0 * size));
			_nodes[6] = new JellyVertex(new JellyVertex(pos.X + -1 * size, pos.Y + 0 * size));
			_nodes[7] = new JellyVertex(new JellyVertex(pos.X + 1 * size, pos.Y + 0 * size));
			_nodes[8] = new JellyVertex(new JellyVertex(pos.X + 3 * size, pos.Y + 0 * size));
			_nodes[9] = new JellyVertex(new JellyVertex(pos.X + -4 * size, pos.Y + -1 * size));
			_nodes[10] = new JellyVertex(new JellyVertex(pos.X + -2 * size, pos.Y + -1 * size));
			_nodes[11] = new JellyVertex(new JellyVertex(pos.X + 0 * size, pos.Y + -1 * size));
			_nodes[12] = new JellyVertex(new JellyVertex(pos.X + 2 * size, pos.Y + -1 * size));
			_nodes[13] = new JellyVertex(new JellyVertex(pos.X + 4 * size, pos.Y + -1 * size));
            ...
            _cellNodes = new int[] { 5, 6, 7, 8 };
            _extverid = new int[] { 5, 6, 7, 8 };

            short[] tri =
            {
                ExtVIdx(10, 0), ExtVIdx(10, 1),
                ExtVIdx(10, 2), ExtVIdx(10, 3),
                ExtVIdx(10, 3),
                ExtVIdx(10, 3), ExtVIdx(10, 2),
                ExtVIdx(10, 1), ExtVIdx(10, 0),
                ExtVIdx(10, 0)
            };
            short[] e_tri = 
            {
                EgeVIdx(1), EgeVIdx(8), ExtVIdx(10, 0),
                EgeVIdx(1), EgeVIdx(8), ExtVIdx(10, 1),
